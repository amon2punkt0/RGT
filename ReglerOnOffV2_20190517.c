...
/* PID TIMEBASE (dt) -----------------------------------------------------------*/
#define CTRL_TIMEBASE	(               10 )	/* dt = 20ms					*/
/* CONTROL FUNCTION	------------------------------------------------------------*/
...
void Control_Height(void) {
	...
	uint32_t duty = 0;
	double delta = 0;
	static int16_t sollDistanz = 150;
	double r1, s0,	s1;
	...
	int32_t b=5300; //set b to the offset the system
	double h=0.01;//time interval
	static double k=0.6;
	if (time % 200==0){
		if (sollDistanz == 150){
			sollDistanz = 150;//100;
	...
	de = (currentError-previousError)/h;
	double bc=600;

	if(currentError > -(de*abs((int32_t)de)/(2*k*b))){
		ctrl = -bc;
	}else{
		ctrl = bc;
	}

	ctrl += b;//Offset für "Schweben" dazuzählen

	//ctrl=b; //experiment to find offset
